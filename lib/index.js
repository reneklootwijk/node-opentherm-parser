...
function toFloat (val) {
  logger.silly(`Parser.toFloat: Entering with ${val}`);
...
function convert (msg, data) {
  ...
  logger.silly(`Parser.convert: Entering with ${JSON.stringify(msg)} and ${data}`);
...
module.exports = data => {
  ...
  logger.silly(`Parser: Entering with ${data}`);

  if (data.length !== 8) {
    logger.error('Parser: Invalid length of Opentherm message');
  ...
  return {
    msgType: { code: msgType, text: msgTypes[msgType] },
    dataId: { id: dataId, description: dataIds[dataId] ? dataIds[dataId].description : 'Unknown' },
    properties
  };
};
